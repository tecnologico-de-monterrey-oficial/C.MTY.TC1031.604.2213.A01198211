...
public:
  BSTBL();
  void insert(T data);
  BoatLog find(string data);
  void remove(T data);
  void print();
  bool isEmpty();
  void order(int type);
  int whatLevelIAm(
      T data); // Regresa el nivel donde se encuentra el dato (root es nivel 1)
};

template <class T> BSTBL<T>::BSTBL() { root = nullptr; }

template <class T> int BSTBL<T>::howManyChilds(NodeT<T> *aux) {
...
template <class T> void BSTBL<T>::insert(T data) {
  ...
  if (isEmpty()) {
    ...
    NodeT<T> *aux = root;
    // Recorremos el arbol hasta encontrar donde colocar el nodo nuevo (while
    // aux != nullptr)
    while (aux != nullptr) {
...
template <class T> BoatLog BSTBL<T>::find(string data) {
  ...
  // NO LO ENCONTRAMOS y 2regresamos falso
  throw std::out_of_range("el dato no existe");
}

template <class T> void BSTBL<T>::remove(T data) {
    ...
    if (data == root->data) {
      ...
      case 2: // tiene 2 hijos
        ...
        if (auxBigLeft->right == nullptr) {
          ...
          // Apuntamos el apuntador del lado izquierdo de aux al lado izquierdo
          // de auxBigLeft
          ...
          NodeT<T> *auxPa = aux;
          // Recorremos hacia la derecha los hijos de auxBigLeft mientras que el
          // apuntador del lado derecho no sea nulo
          ...
          // Hacer el cambio de auxBigLeft con aux
          aux->data = auxBigLeft->data;
          // Apuntamos el apuntador del lado derecho de auxPa al lado izquierdo
          // de auxBigLeft
      ...
      while (aux != nullptr) {
        ...
        if (aux->data == data) {
          ...
          case 1: // Tiene 1 hijo
            ...
            if (data < auxPa->data) {
              aux->left != nullptr ? auxPa->left = aux->left
                                   : auxPa->left = aux->right;
            } else {
              aux->left != nullptr ? auxPa->right = aux->left
                                   : auxPa->right = aux->right;
            }
            // Removemos aux
            delete aux;
            break;
          case 2: // Tiene 2 hijos
            ...
            // Creamos un apuntador auxiliar que apunte al lado izquierdo de aux
            NodeT<T> *auxBigLeft = aux->left;
            // Validamos si el apuntador del lado    derecho de auxBigLeft es
            // nulo
            if (auxBigLeft->right == nullptr) {
              // auxBigLeft es el más grande del lado izquierdo de aux
              // Hacer el cambio de auxBigLeft con aux
              aux->data = auxBigLeft->data;
              // Apuntamos el apuntador del lado izquierdo de aux al lado
              // izquierdo de auxBigLeft
              aux->left = auxBigLeft->left;
              // borramos AuxBigLeft
              delete auxBigLeft;
            } else {
              // Creamos un apuntado auxliar que va a ser el papa de auxBigLeft
              NodeT<T> *auxPa = aux;
              // Recorremos hacia la derecha los hijos de auxBigLeft mientras
              // que el apuntador del lado derecho no sea nulo
              while (auxBigLeft->right != nullptr) {
                // Recorremos el papa de auxBigLeft
                auxPa = auxBigLeft;
                // Recorremos a la derecha auxBigLeft
              ...
              // Hacer el cambio de auxBigLeft con aux
              aux->data = auxBigLeft->data;
              // Apuntamos el apuntador del lado derecho de auxPa al lado
              // izquierdo de auxBigLeft
              auxPa->right = auxBigLeft->left;
              // borramos AuxBigLeft
              delete auxBigLeft;
      ...
      throw invalid_argument("El valor a borra no se encuentra en el BSTBL");
    }
  } else {
    throw invalid_argument("El valor a borra no se encuentra en el BSTBL");
  }
...
template <class T> void BSTBL<T>::printTree(NodeT<T> *aux, int level) {
  if (aux != NULL) {
...
template <class T> void BSTBL<T>::preOrder(NodeT<T> *aux) {
...
template <class T> void BSTBL<T>::inOrder(NodeT<T> *aux) {
  // Condición de control para salir de la recursividad
  if (aux != nullptr) {
...
template <class T> void BSTBL<T>::postOrder(NodeT<T> *aux) {
  // Condición de control para salir de la recursividad
  if (aux != nullptr) {
    // inOrder del arbol izquierdo
    postOrder(aux->left);
...
template <class T> void BSTBL<T>::preOrderConverse(NodeT<T> *aux) {
  // Condición de control para salir de la recursividad
  if (aux != nullptr) {
    // visita el nodo raíz del arbol
    cout << aux->data << " ";
...
template <class T> void BSTBL<T>::inOrderConverse(NodeT<T> *aux) {
  // Condición de control para salir de la recursividad
  if (aux != nullptr) {
    // inOrder del arbol derecho
...
template <class T> void BSTBL<T>::postOrderConverse(NodeT<T> *aux) {
  // Condición de control para salir de la recursividad
  if (aux != nullptr) {
    // inOrder del arbol derecho
...
template <class T> void BSTBL<T>::levelByLevel() {
  ...
  while (!queue.isEmpty()) {
    // Creamos un nodo auxliar para almacenar el valor del nodo a sacar de la
    // fila
...
template <class T> void BSTBL<T>::order(int type) {
  switch (type) {
  case 1: // 1 -> PreOrder
    preOrder(root);
    break;
...
template <class T> void BSTBL<T>::print() {
  if (!isEmpty()) {
    int level = 0;
    cout << endl;
    ...
    cout << endl << "The BSTBL is empty" << endl << endl;
  }
}

template <class T> bool BSTBL<T>::isEmpty() { return root == nullptr; }
#endif
