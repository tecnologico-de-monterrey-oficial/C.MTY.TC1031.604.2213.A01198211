...
template <class T> void busquedaBinaria(vector<T> list, T x) {
  int size = list.size();
  int key;
  int left = 0;
  int right = size - 1;
...
// swap sort O(n^2)
template <class T> void swapSort(vector<T> &list) {
  ...
  cout << endl
       << "comparasiones: " << comparisons << " Intercambios: " << swaps
       << endl;
};
...
template <class T> void selectionSort(vector<T> &list) {
  int swaps = 0;
  int comparisons = 0;
  for (int i = 0; i <= list.size() - 1; i++) {
    T item = list[i];
...
// InsertionSort O(n^2)
template <class T> void insertionSort(vector<T> &list) {
  int swaps = 0;
  int comparisons = 0;
...
template <class T> void printList(vector<T> &list) {
  for (auto el : list) {
    cout << el << " ";
  }
  cout << endl;
...
template <class T> void merge(vector<T> &list, int inf, int mid, int sup) {
  ...
  // Hacemos un ciclo mientras el índice de la izquierda < al tamaño de la lista
  // izquierda y (&&) el índice de la derecha < al tamaño de la lista derecha
  while (idxLeft < left.size() && idxRight < right.size()) {
    // Comparamos si el valor que se encuentra en el índice de la izquierda
    // (idxLeft) es menor al valor que se encuentra en el índice de la derecha
    // (idxRight)
    if (left[idxLeft] < right[idxRight]) {
      // Si es menor
      // asignamos el valor de la lista izquierda que se encuentra en la
      // posición del índice izquierdo (idxLeft) en la posición de índice de la
      // lista (idx)
      ...
      // No es menor
      // asignamos el valor de la lista derecha que se encuentra en la posición
      // del índice derecho (idxRight) en la posición de índice de la lista
      // (idx)
  ...
  while (idxLeft < left.size()) {
    // asignamos el valor de la lista izquierda que se encuentra en la posición
    // del índice izquierdo (idxLeft) en la posición de índice de la lista (idx)
    list[idx] = left[idxLeft];
    // incrementar el índice de la lista (idx)
    idx++;
    // incrementar el índice de la lista izquierda (idxLeft)
  ...
  // Hacemos un ciclo mientras el índice de la derecha < al tamaño de la lista
  // derecha
  while (idxRight < right.size()) {
    // asignamos el valor de la lista derecha que se encuentra en la posición
    // del índice derecho (idxRight) en la posición de índice de la lista (idx)
    list[idx] = right[idxRight];
    // incrementar el índice de la lista (idx)
    idx++;
    // incrementar el índice de la lista derecha (idxRight)
...
template <class T> void mergeSort(vector<T> &list, int inf, int sup) {
  ...
  if (inf < sup) {
    ...
    int mid = (inf + sup) / 2;
    // Volver a llamar a la función con la sublista del lado izquierdo
    // (list,inf,mid)
    mergeSort(list, inf, mid);
    // Volver a llamar a la función con la sublista del lado derecho
    // (list,mid+1,sup)
    mergeSort(list, mid + 1, sup);

    // Ocurre la magia - Juntar la sublista izquierda con la sublista derecha
    // que ya estan ordenadas
    merge(list, inf, mid, sup);
  }
}
// Funcion para crear el vector O(n)
template <class T> void crearvector(vector<T> &lista, int size) {
  T elemento;
...
int main() {
  ...
  while (true) {
    char option;
    cout << "a) crear listas" << endl
         << "b) busqueda secuencial" << endl
         << "c) busqueda binaria" << endl
         << "d) Bubble sort" << endl
         << "e) selection sort" << endl
         << "f) insertion sort" << endl
         << "g)MergeSort" << endl
         << "h) Swap sort" << endl
         << "i) quick sort" << endl
         << "q) cerrar programa" << endl;
    cin >> option;
    if (option == 'a') {
      int size;
